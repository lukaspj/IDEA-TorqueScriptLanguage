...
class EngineClassPropertyParser(eventReader: XMLEventReader, scopeList: List<String>) {
    fun parse(event: StartElement): EngineClassProperty {
        ...
        val isConstant = event.getAttributeByName(QName("", "isConstant")).value!!
        val isTransient = event.getAttributeByName(QName("", "isTransient")).value!!
        val isVisible = event.getAttributeByName(QName("", "isVisible")).value!!
