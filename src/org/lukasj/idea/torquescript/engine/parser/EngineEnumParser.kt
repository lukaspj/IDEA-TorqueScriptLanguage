...
class EngineEnumParser(private val eventReader: XMLEventReader, private val scopeList: List<String>) {
        ...
        while (eventReader.hasNext()) {
            ...
            if (nextEvent.isStartElement) {
                ...
                when (nextStartEvent.name.localPart) {
                    ...
                    else -> throw Throwable("Unexpected Engine Enum child ${nextStartEvent.name.localPart} ${nextStartEvent.location.lineNumber}")
                }
            }
            if (nextEvent.isEndElement && nextEvent.asEndElement().name.localPart == "EngineEnumType") {
